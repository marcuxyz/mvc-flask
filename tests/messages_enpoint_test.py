...
#     def test_return_status_200_for_index(self):
#         response = self.client.get(url_for(f"home.index"))
...
# @test("should update message from form", tags=["request", "messages", "update"])
...
# @test("should delete message from form", tags=["request", "messages", "delete"])
# def _(browser=browser):
#     message = Message(title="Message One")
#     db.session.add(message)
#     db.session.commit()
