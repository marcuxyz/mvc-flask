...
class TimerUtil:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.max_duration and self.duration > self.max_duration:
            pytest.fail(f"Operation took {self.duration:.3f}s, expected < {self.max_duration}s")
...
class ValidationHelper:
    ...
    def validate_restful_routes(app: Flask, resource_name: str):
        ...
        for expected_rule, expected_methods in expected_routes.items():
            assert expected_rule in route_dict, \
                f"Route '{expected_rule}' not found"

            for method in expected_methods:
                assert method in route_dict[expected_rule], \
                    f"Method '{method}' not found for route '{expected_rule}'"

    @staticmethod
    def validate_blueprint_structure(app: Flask, blueprint_name: str, expected_endpoints: List[str]):
        ...
        for expected_endpoint in expected_endpoints:
            ...
            assert full_endpoint in actual_endpoints, \
...
class BenchmarkHelper:
    ...
    def compare_performance(func1: Callable, func2: Callable, iterations: int = 100) -> Dict[str, Any]:
        ...
        return {
            ...
            'winner': 'function2' if stats2['avg_time'] < stats1['avg_time'] else 'function1'
        }
